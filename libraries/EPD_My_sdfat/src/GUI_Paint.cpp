...
******************************************************************************/
void Paint_DrawChar(UWORD Xpoint, UWORD Ypoint, const unsigned char Acsii_Char,
                    sFONT* Font, UWORD Color_Background, UWORD Color_Foreground)
{
    ...
    unsigned char ch=Acsii_Char;
    if(ch==0xBB || ch==0xAB)ch=0x22;
uint32_t offset=(ch - ' ');
if(ch>'~'){
	offset=('~'-' ')+(ch-0xC0)+1;
}

    uint32_t Char_Offset = offset * Font->Height * (Font->Width / 8 + (Font->Width % 8 ? 1 : 0));
...
parameter:
    ...
    Color_Foreground : Select the foreground color of the English character
******************************************************************************/
void Paint_DrawString_EN(UWORD Xstart, UWORD Ystart, const unsigned char * pString,
                         sFONT* Font, UWORD Color_Background, UWORD Color_Foreground )
{
    UWORD Xpoint = Xstart;
    UWORD Ypoint = Ystart;
